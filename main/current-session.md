...
- ⬜ **mpaj-icomm: `main` vs `main-production` reconciliation** — full diff done Aug 13 (Hana/Nadia/Reza), real gaps found (see Compacted History), reconciliation itself not started. FT mode — Hakim's to merge/commit.
...
### Aug 13, 2026 — FT mode · mpaj-icomm branch drift + homelab build-out
- **`main` vs `main-production` diff (25 files)**: main is meaningfully behind — real feature gap (`alamat` fields never wired into `maklumat-profil-syarikat` on main + a live `dd($e)` debug call that would halt requests), a real payment bug fix missing (`semakPembayaranSenangpay()` zero/null `registration_fee` fallback), 3 submodule providers disabled on main that are live on production, plus several diverged spots needing Hakim's manual call (barcode-scan guard regression risk, BOQ matching mechanics, EPelesenan menu removal intent). Reza found a trivial byte-corruption bug in production's `SsoController.php` (stray `x` before `<?php`). Investigation only, no reconciliation yet.
...
- **SMS/TAC year-old bug fully closed (Aug 6-11)**: root cause was outbound firewall + a commented-out `proxy_pass` on the WAF box, both fixed; final "false" test result traced to an unrelated log-file permission issue (tinker runs as Hakim's shell user, today's log was created by `www-data`), not a real pipeline failure. Confirmed live via organic real-user 201s from Celcom.
- **Hebahan edit-save bug root-caused**: `SenaraiHebahan.php` has no `use Exception;` import, so `catch (Exception $e)` silently fails to catch anything (resolves to a non-existent namespaced class) — hits all 3 environments identically. Fix applied locally, not committed (FT mode). Bigger finding: only bill-type "T" has real batch-blast logic; S/K/M/L (4 of 5 dropdown options) have none — product decision needed.
