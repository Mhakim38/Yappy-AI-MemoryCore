...
- **More renderer gotchas (found Jul 29, dark-mode mockup session)**: `render-batch` ALWAYS auto-positions every frame via its own smart-positioning (ignores any `x`/`y` in the JSX, hardcodes them based on max-right of existing content) — for exact absolute positioning, use single `render` calls instead (that code path honors `props.x`/`props.y` directly and skips smart-positioning when `x` is present). A `<Frame>` with no `bg`/`fill` renders fully transparent, not white. `justify="end"` vs `"start"` on a `flex="col"` frame controls which end of the vertical axis content anchors to — easy to get backwards (caused a bottom-sheet drawer to render stuck to the top instead of the bottom, caught via `verify` screenshot and fixed). Dashed borders are NOT supported by the renderer (no `strokeDash` prop) — solid border is the fallback for dropzone-style UI. Per-corner radius (`roundedTL`/`TR`/`BL`/`BR`) works fine for rounded-top-only shapes like bottom sheets.
